...
Global preferences for AI coding agents (Claude Code, Cursor, Antigravity, etc.).
...
Custom skills live in `~/.gemini/antigravity/skills/` (SKILL.md format: YAML frontmatter + instructions). Scripts to be ported there from `.claude/skills/`. Until ported, invoke via terminal.
...
### document (Obsidian)
Log the current session to the Obsidian vault. Obsidian MCP is **not yet configured** in Antigravity (`~/.gemini/antigravity/mcp_config.json` is empty) — invoke via terminal for now.

---

## Antigravity-Specific

- Global rules: this file lives at `~/.gemini/GEMINI.md` and applies to all workspaces
- Project rules: `.antigravity/rules.md` in the project root for project-specific overrides
- Custom skills: `~/.gemini/antigravity/skills/` — SKILL.md format (YAML frontmatter + instructions); strava/sheets/calendar skills to be ported there
- MCP servers: `~/.gemini/antigravity/mcp_config.json` — currently empty (Obsidian MCP not yet configured)
